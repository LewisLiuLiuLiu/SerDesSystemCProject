...
## 概述
差分输入/差分输出的可变增益放大器（VGA），采用多零点/多极点的传递函数。支持可配置增益、输入偏移、输入端噪声注入（可开关）、输出饱和限制，以及可配置的差分输出共模电压。
...
## 参数
- 主传递函数：多零点与多极点（`zeros`/`poles`），配合 `dc_gain` 构成 `H_vga(s)`
...
2. 主线性滤波：根据 `zeros, poles, dc_gain` 构造 `H_vga(s)`，使用 `sca_tdf::sca_ltf_nd` 进行时域滤波，得到 `vout_diff_l`。
...
- 建模域建议：VGA/PSRR/CMRR 推荐使用 TDF 的 `sca_tdf::sca_ltf_nd`；CMFB 闭环如需更自然的连续实现可采用 LSF/ELN

## 使用示例
1. 在 RX 的 VGA 阶段配置：`dc_gain`, `zeros`, `poles`, `vcm_out`, `sat_min/sat_max`
...
6. 增益调节：通过动态修改 `dc_gain` 实现可变增益功能，典型应用于 AGC（自动增益控制）环路中

## 测试验证
- 频响一致性：Bode 幅相响应与目标零极点模型一致
- 增益可变性：验证 `dc_gain` 参数变化时输出增益的线性对应关系
...
## 变更历史
- v0.1（2025-10-22）：初始版本，参照 CTLE 模块文档结构；统一以 `vcm_out` 作为共模设定点与 CMFB 目标；支持 PSRR/CMFB/CMRR 非理想效应建模；遵循 `zeros`/`poles` 数组配置规范
